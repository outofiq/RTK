...
int DecodeNovOem7Dat(unsigned char Buff[], int& Len, epoch_t* obs, gpseph_t geph[], gpseph_t beph[], epochPOS_t* epos)
{
	...
	int flag = 0;// 读到观测值的标志(读到观测值类型的message就需要退出循环来定位解算,此时flag=1)
	double pos[3];// 用于保留obs中接收机定位结果的辅助量
	int msg_ID;// 判断一条message存放着何种类型的数据（obs/eph/pos）
	...
	while (1)
	{
		...
		// 进行头文件完整性判断
		if ((i + OEM7HLEN) > Len) break;// 若头文件不完整则退出循环
		...
		// 核验crc码
		unsigned int crc = rtkCRC32(buff, len);
		if (crc != U4(buff + len))
		{
			i = i + 1;
		...
		case ID_RANGE:
			...
			flag = decodeRangeb(buff, obs);//此处将flag赋值为1，表示读取到观测值消息
			break;

		case ID_GPSEPHEM:
			decodeGpsEphem(buff, geph);//传数组名
			break;

		case ID_BDSEPHEM:
			decodeBdsEphem(buff, beph);//传数组名
			break;

		case ID_BESTPOS:
			decodeBestpos(buff, epos->readpos);
			break;
...
int decodeRangeb(unsigned char* buff, epoch_t* obs) 
{
	...
	for (int i = 0; i < obs_num; i++)
	{
		...
		if (sys == 0) satsys = sys_t::GPS;
		else if (sys == 4) satsys = sys_t::BDS;
		else continue;
		...
		{
			if (satsys == obs->SatObs[j].Sys && prn == obs->SatObs[j].Prn)
			{
				index = j;
			}
			else
...
**********************************************************
*/
int decodeGpsEphem(unsigned char* buff, gpseph_t* geph)//首地址
{
	...
	// 判断数组是否会越界
	if (prn > MAXGPSPRN || prn < 1) return 0;

	eph->health = U4(buff + OEM7HLEN + 12);// 0-正常，1-不正常
	...
	eph->ecc = R8(buff + OEM7HLEN + 64);
	eph->M0 = R8(buff + OEM7HLEN + 56);// rad
	eph->OMEGA0 = R8(buff + OEM7HLEN + 144);// rad
...
int decodeBdsEphem(unsigned char* buff, gpseph_t* beph)
{
	...
	// 判断数组是否会越界
	if (prn > MAXBDSPRN || prn < 1) return 0;

	eph->health = U4(buff + OEM7HLEN + 16);// 0-正常，1-不正常
	...
	eph->ecc = R8(buff + OEM7HLEN + 84);
	eph->M0 = R8(buff + OEM7HLEN + 108);// rad
	eph->OMEGA0 = R8(buff + OEM7HLEN + 116);// rad
...
参数：buff        存有一条完整的定位结果messgae的字符串首地址
	  readpos	  读取到的定位结果数据的存储体
返回值：星历数据是否解码成功（1-成功，0-失败）
函数功能：对buff中的数据解码，并存储在readpos中
**********************************************************
*/
int decodeBestpos(uint8_t* buff, BLH &readpos)
{
	readpos.B = R8(buff + OEM7HLEN + 8) * D2R;//经度(rad)
	readpos.L = R8(buff + OEM7HLEN + 16) * D2R;//纬度(rad)
	readpos.H = R8(buff + OEM7HLEN + 24) + R4(buff + OEM7HLEN + 32);//大地高(m)
	return 1;
}